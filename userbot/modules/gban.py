...
async def gspider(userbot):
    ...
    try:
        ...
        return await friday.edit(f"**There is an error!!**")
    if user:
    ...
    try:
        if gmute(user.id) is False:
            return await friday.edit(f"**Error! User has been gbanned.**")
    except BaseException:
        pass
    return await friday.edit(
        f"**Gbanned [{user.first_name}](tg://user?id={user.id}) from : {a} Group**"
    )
